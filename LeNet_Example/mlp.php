...
class HiddenLayer {
	...
	var $W			= false;
	var $b			= false;
	var $activation	= false;	// Нелинейность используемая в скрытом слое

	function Init($input, $n_in, $n_out, $W=false, $b=false, $activation=false) {
		/*
		...
		if(!$W) {
			...
			for($i=0; $i<$this->n_in; $i++) {
				for ($j=0; $j<$this->n_out; $j++) {
					$this->rng[$i][$j] = rand($low*100, $hight*100)/100;
				}
			}
